...
private:
    ...
    int HistogramParamName(Person p, int gap) {
        string lastName = p.getLastName();
        char firstLetter = lastName[0];
        if (firstLetter < 'A' || firstLetter > 'Z')
            throw out_of_range("Wrong Last Name!");
        int category = 1;
...
public:
    ...
    void LastNameHistogram(Sequence<Person>* seq, int gap) {
        int amount;
